...
export async function GET(request: Request) {
    ...
    if(nameFilter) {
        const filteredClients = await db.select().from(client).where(or(
            ilike(client.firstName, `${nameFilter}%`),
            ilike(client.lastName, `${nameFilter}%`)
        )).orderBy(client.firstName, client.lastName).limit(filterLimit);
...
export async function POST(request: Request) {
    ...
    const newClientId = await db.insert(client).values(newClient).returning({ id: client.id });
