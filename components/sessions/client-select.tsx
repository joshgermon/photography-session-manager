...
export function ClientSelect(props: ClientSelectProps) {
  ...
  async function fetchSuggestions(input: string) {
    const filterClients = await fetch(`/api/customers?filter=${input}&limit=5`);
    const filteredClients = (await filterClients.json()) as Client[];
          ...
          {suggestions.length > 0 ? (
            suggestions.map((item, i) => (
              <div
                ...
                className={`w-full px-3 py-2 text-sm text-gray-600 focus:bg-slate-200 hover:bg-slate-200 ${
                  active === i && "bg-slate-200"
                }`}
                key={item.id}
              >
